...
function ProductForm({ product, image, handleClose }) {
  ...
  const displayAttCategories = (product) => {
    if (product.product_attribute) {
      return (
        <div>
          {JSON.parse(product.product_attribute).map((id) => {
            return (
              <ProductAttribute
                key={id}
                paId={id}
                ...
                n={JSON.parse(product.product_attribute).length}
              />
            );
          })}
        </div>
  ...
  const calculateTotal = (selections) => {
    ...
    if (product.product_attribute) {
      if (
        fullSelection.length == JSON.parse(product.product_attribute).length
      ) {
  ...
  return (
    <div>
      ...
      <Modal.Body>
        ...
        <div>{displayAttCategories(product)}</div>
      </Modal.Body>
      <Modal.Footer
